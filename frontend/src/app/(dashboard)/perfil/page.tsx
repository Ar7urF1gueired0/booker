             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm">
                ...
                <div className="flex gap-2">
                   {friendsList.slice(0, 4).map(friend => (
                      <div key={friend.id} className="relative w-10 h-10 rounded-full border-2 border-white shadow-sm overflow-hidden bg-gray-100">
             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                   <div className="relative w-10 h-10 rounded-full overflow-hidden bg-gray-200">
                      <Image src="https://i.pravatar.cc/150?u=amanda" alt="Amanda" fill className="object-cover"/>
                   </div>
                   <div>
      ...
      {activeTab === 'Amigos' && (
        <div className="bg-white p-6 rounded-3xl shadow-sm animate-in fade-in duration-300">
           <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
             Todos os Amigos <span className="text-sm font-normal text-gray-400">({friendsList.length})</span>
           </h2>
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {friendsList.map(friend => (
                <div key={friend.id} className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-green-200 hover:shadow-md transition bg-gray-50/50">
                   <div className="relative w-12 h-12 rounded-full overflow-hidden bg-gray-200">
                      <Image src={friend.img} alt={friend.name} fill className="object-cover" />
                   </div>
                   <div>
                      <p className="font-bold text-gray-800">{friend.name}</p>
                      <button className="text-xs text-green-600 font-semibold hover:underline">Ver Perfil</button>
                   </div>
                </div>
              ))}
           </div>
      ...
      {activeTab === 'Conquistas' && (
        <div className="space-y-4 animate-in fade-in duration-300">
           <div className="flex gap-4 mb-6 overflow-x-auto pb-2">
             <button className="bg-green-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-sm">Todos</button>
             <button className="bg-white text-gray-600 px-4 py-2 rounded-full text-sm font-bold shadow-sm hover:bg-gray-50">Pro</button>
             <button className="bg-white text-gray-600 px-4 py-2 rounded-full text-sm font-bold shadow-sm hover:bg-gray-50">Amador</button>
           </div>

           {achievements.map((item) => (
             <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              ...
              <button onClick={() => setIsEditing(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-gray-100">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" defaultValue={user?.fullName} className="w-full p-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <div className="flex gap-2">
                       <button type="button" className="flex-1 py-2 bg-green-100 text-green-700 font-bold rounded-lg border border-green-200 text-sm">Direita</button>
                       <button type="button" className="flex-1 py-2 bg-gray-50 text-gray-500 font-medium rounded-lg border border-gray-200 hover:bg-gray-100 text-sm">Esq.</button>
                    </div>
                 </div>
                 <div>
